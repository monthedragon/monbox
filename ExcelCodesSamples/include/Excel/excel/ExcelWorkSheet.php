...
class ExcelWorkSheet {
	...
	/**
	 * setting the sheet scale
	 * updating this sheet part
	 * <sheetViews>
	 *      <sheetView tabSelected="1" zoomScale="{$scale}" zoomScaleNormal="{$scale}" workbookViewId="0">
	...
	public function setZoomScale($scale) {
		...
		$sheetView = $d->getElementsByTagName('sheetView')->item(0);
		//im not sure if sheetView is always available. but based on some templates this is default in sheets
		//insert here the code if the $sheetView is not present in the current sheet xml

		//IF $sheetView is present
		$sheetView->setAttribute("zoomScale", "$scale");
		$sheetView->setAttribute("zoomScaleNormal", "$scale");
		$this->newSheetsTemplate[$this->currentSheet] = $d->saveXML();
	}
