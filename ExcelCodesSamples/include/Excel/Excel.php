...
class Excel
{
    ...
    public function __construct($template_path = false){
        $this->excelFolder = get_cache_temp_path()."excel";
...
//        $this->util->cleanExcelTempFolder($this->excelFolder);
    ...
    protected function load($path) {
        //Jed BR 82089  / 81968  Added the unique Id with the Time to prevent mixup of template when two users press the button on different screens at the same time.
        $id = create_guid();
        // GET CURRENT Login ID of User
        global $current_user;
        $this->excelFolder = $this->excelFolder.'/'.$current_user->id.$id.time();
	...
	/**
	 * sets the current sheet zoom scale
	 *
	 * @param (int) $zoomScale zoom scale of the sheet (this is in percent(%) datatype)
	 * TR 87756
	 */
	public function setSheetZoomScale($zoomScale) {
		$zoomScale = (int) $zoomScale;
		$this->worksheet->setZoomScale($zoomScale);
	}
}
